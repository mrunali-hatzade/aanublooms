...
export const CustomOrderBuilder = ({ onNavigate }) => {
  ...
  const estimatedPrice = currentTypeObj.priceBase + (stemCount === 'Deluxe 15 Stems' ? 25 : 0);
  ...
  const handleInquirySubmit = async (e) => {
    ...
    try {
      await api.submitCustomRequest({
        ...
        estimatedBudget: `$${estimatedPrice.toFixed(2)}`
      });
    ...
    <div className="py-12 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="text-center max-w-2xl mx-auto mb-10">
        ...
        <p className="text-sm text-warmgray-600 dark:text-warmgray-400 mt-2">
          Personalize yarn colors, flower combinations, sizes, and personalized ribbon messages handcrafted specially for you or a loved one.
        </p>
      </div>

      {/* Step Indicators */}
      <div className="flex items-center justify-center gap-2 sm:gap-4 mb-10 text-xs font-bold">
        {[
      ...
      {step === 1 && (
        <div className="space-y-6 animate-in fade-in">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {creationOptions.map(option => (
              <div
                ...
                className={`p-5 rounded-3xl border-2 cursor-pointer transition-all flex gap-4 ${
              ...
              >
                <img src={option.image} alt={option.name} className="w-24 h-24 rounded-2xl object-cover shrink-0" />
                <div className="flex flex-col justify-between">
                  <div>
                    <h3 className="font-serif font-bold text-base text-warmgray-900 dark:text-white leading-snug">
                      {option.name}
                    </h3>
                    <p className="text-xs text-warmgray-500 dark:text-warmgray-400 line-clamp-2 mt-1">
                      {option.desc}
                    </p>
                  </div>
                  <div className="flex items-center justify-between mt-3">
                    <span className="text-sm font-serif font-bold text-bloom-600 dark:text-bloom-400">
                      Starting at ${option.priceBase.toFixed(2)}
                    </span>
                    {creationType === option.name && (
                      <div className="w-6 h-6 rounded-full bg-bloom-500 text-white flex items-center justify-center">
          ...
          <div className="flex justify-end pt-4">
            <button
              onClick={() => setStep(2)}
              className="px-8 py-3.5 bg-bloom-500 hover:bg-bloom-600 text-white rounded-full font-bold text-xs shadow-cozy transition-all"
            >
      ...
      {step === 2 && (
        <div className="bg-white dark:bg-warmgray-900 rounded-3xl p-6 sm:p-8 border border-warmgray-200/80 dark:border-warmgray-800 shadow-soft space-y-8 animate-in fade-in">
          
          {/* Color Palette Selector */}
          <div>
            <div className="flex justify-between items-center mb-3">
              <label className="font-serif font-bold text-base text-warmgray-900 dark:text-white">
                ...
                return (
                  ...
                  >
                    <span
                      ...
                      className="w-6 h-6 rounded-full border border-warmgray-300 dark:border-warmgray-600 shrink-0 shadow-xs"
                    />
          ...
          {creationType.includes('Bouquet') && (
            <div>
              <label className="font-serif font-bold text-base text-warmgray-900 dark:text-white block mb-3">
                ...
                {flowerOptions.map(flower => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-4 py-2 rounded-2xl text-xs font-semibold border transition-all flex items-center gap-1.5 ${
                        isSelected
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {['100% Combed Milk Cotton', 'Super-Soft Chenille Velvet', 'Organic Bamboo Cotton'].map(yarn => (
                <button
                  ...
                  className={`p-3.5 rounded-2xl text-xs font-semibold border text-center transition-all ${
                    yarnPreference === yarn
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-warmgray-800 dark:text-warmgray-200 mb-1.5">
            ...
            <div>
              ...
              >
                ...
                <option value="Deluxe 15 Stems">Deluxe Luxe (15 Stems) (+$25)</option>
          ...
          <div>
            <label className="block text-xs font-bold text-warmgray-800 dark:text-warmgray-200 mb-1.5">
              Additional Notes or Reference Links for Artisan Aanu
            </label>
            <textarea
              rows={3}
              value={specialNotes}
          ...
          <div className="flex justify-between items-center pt-4 border-t border-warmgray-100 dark:border-warmgray-800">
            <button
              onClick={() => setStep(1)}
      ...
      {step === 3 && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in">
          ...
          {/* Summary Box */}
          <div className="lg:col-span-7 bg-white dark:bg-warmgray-900 rounded-3xl p-6 sm:p-8 border border-warmgray-200/80 dark:border-warmgray-800 shadow-soft space-y-6">
            <h3 className="font-serif font-bold text-xl text-warmgray-900 dark:text-white">
              Bespoke Commission Summary
            </h3>

            <div className="flex gap-4 p-4 rounded-2xl bg-warmgray-50 dark:bg-warmgray-800/60 border border-warmgray-100 dark:border-warmgray-700">
              <img src={currentTypeObj.image} alt="preview" className="w-20 h-20 rounded-xl object-cover" />
              <div>
            ...
            <div>
              <p className="text-xs font-bold text-warmgray-700 dark:text-warmgray-300 mb-2">
                Custom Yarn Palette:
              </p>
              <div className="flex gap-2">
                {selectedColors.map((c, idx) => (
                  <div key={idx} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-warmgray-100 dark:bg-warmgray-800 border border-warmgray-200 dark:border-warmgray-700 text-xs">
                    <span style={{ backgroundColor: c }} className="w-4 h-4 rounded-full border" />
            ...
            {/* Estimated Price */}
            <div className="p-4 rounded-2xl bg-gradient-to-r from-bloom-50 to-rosewood-50 dark:from-warmgray-800 dark:to-warmgray-800 border border-bloom-200 dark:border-bloom-800/50 flex justify-between items-center">
              <div>
                <span className="text-xs font-semibold text-warmgray-600 dark:text-warmgray-300">
                  Estimated Handcraft Value
                </span>
                <p className="text-2xl font-serif font-bold text-bloom-600 dark:text-bloom-400">
            ...
            <div className="space-y-3 pt-2">
              {/* Option 1: Direct Instant Checkout */}
              <button
                onClick={handleAddToCart}
                className="w-full py-4 bg-gradient-to-r from-bloom-500 to-rosewood-500 hover:from-bloom-600 hover:to-rosewood-600 text-white rounded-2xl font-bold text-sm shadow-cozy flex items-center justify-center gap-2 transition-all transform hover:scale-[1.01]"
              >
                <ShoppingBag className="w-4 h-4" />
                <span>Add to Basket & Proceed to Checkout (${estimatedPrice.toFixed(2)})</span>
              </button>
          ...
          {/* Form for Inquiry Inquiry */}
          <div className="lg:col-span-5 bg-warmgray-50 dark:bg-warmgray-900/60 rounded-3xl p-6 sm:p-8 border border-warmgray-200/80 dark:border-warmgray-800 space-y-4">
            <h4 className="font-serif font-bold text-lg text-warmgray-900 dark:text-white">
            ...
            <form onSubmit={handleInquirySubmit} className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full text-xs p-3 rounded-xl bg-white dark:bg-warmgray-800 border border-warmgray-200 dark:border-warmgray-700 text-warmgray-900 dark:text-white"
                />
              </div>
      ...
      {step === 4 && (
        <div className="bg-white dark:bg-warmgray-900 rounded-3xl p-8 sm:p-12 text-center max-w-lg mx-auto border border-warmgray-200/80 dark:border-warmgray-800 shadow-soft-lg animate-in zoom-in-95 space-y-4">
          <div className="w-16 h-16 bg-rosewood-100 dark:bg-rosewood-950 text-rosewood-500 rounded-full mx-auto flex items-center justify-center shadow-cozy">
            <Heart className="w-8 h-8 fill-rosewood-500" />
          </div>
          ...
          <button
            ...
            className="px-6 py-2.5 bg-bloom-500 hover:bg-bloom-600 text-white rounded-full font-bold text-xs shadow-cozy mt-4"
          >
